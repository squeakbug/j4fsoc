...
################################################################################################
# https://github.com/openhwgroup/cvw/blob/main/bin/wally-tool-chain-install.sh 
################################################################################################
...
if [ "$ROOT" == true ]; then
    ...
    source "${dir}"/package-install.sh --check
fi
...
echo -e "${SUCCESS_COLOR}${BOLD}\n\nJ4FSOC INSTALLATION SUCCESSFUL!!!\n\n${ENDC}"
